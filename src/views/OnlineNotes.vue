<template>
  <div class="about">
    ...
    <div class="content" v-if="resultArray.length">
...
export default {
  ...
  setup () {
    ...
    const { processFile, resultArray } = useDataFile()
    onMounted(() => {
      if (localStorage.getItem('resultArray')) {
        try {
          resultArray.value = JSON.parse(localStorage.getItem('resultArray'))
        } catch (e) {
          console.log(e)
        }
      }
    ...
    const filterItems = computed(() => {
      return resultArray.value.slice(11).filter((el) => el.count !== '')
    })
    ...
    return {
      ...
      resultArray,
      mark,
      date: computed(() => store.getters.getDateUploadFile ||   JSON.parse(localStorage.getItem('setDateUploadFile')))
    }
  }
}
</script>
